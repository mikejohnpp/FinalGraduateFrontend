...
export default function GroupsSidebar() {
  ...
  return (
    <aside className="hidden md:flex w-70 shrink-0 flex-col border-r bg-background">
      <ScrollArea className="h-full">
        <div className="flex flex-col gap-3 p-3 pb-6">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold">Nhóm</h2>
            <Button variant="ghost" size="icon" className="size-8 rounded-full bg-muted hover:bg-muted/80">
              <Settings data-icon="inline" />
            </Button>
          </div>

          <div className="relative">
            <Search data-icon="inline" className="absolute left-2.5 top-2.5 text-muted-foreground" />
            <Input 
          ...
          <div className="flex flex-col gap-1">
            {navItems.map((item) => {
              const isActive = location.pathname === item.path || (item.path === PATH_CONSTRAINT.GROUPS && location.pathname === "/groups");
              const Icon = item.icon;
              return (
                <Button
                  key={item.path}
                  variant={isActive ? "secondary" : "ghost"}
                  className={cn("h-11 w-full justify-start gap-3 rounded-lg font-medium", isActive ? "bg-muted/80" : "hover:bg-muted/50")}
                  onClick={() => navigate(item.path)}
                >
                  <div className={cn("flex items-center justify-center rounded-full p-1.5", isActive ? "bg-primary text-primary-foreground" : "bg-muted text-foreground")}>
              ...
              {joinedGroups.slice(0, 8).map(group => (
                ...
                >
                  <Avatar className="size-10 rounded-lg">
                    <AvatarImage src={group.coverPhoto} className="object-cover" />
                    <AvatarFallback className="rounded-lg">{group.name.charAt(0)}</AvatarFallback>
                  </Avatar>
                  <div className="flex-1 min-w-0 text-left">
                    <p className="truncate font-semibold">{group.name}</p>
                    {group.lastAccessed && (
                      <p className="truncate text-xs text-muted-foreground">Truy cập {group.lastAccessed}</p>
                    )}
                  </div>
                </Button>
              ))}
