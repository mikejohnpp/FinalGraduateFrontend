...
export default function MessageBubble({
  ...
  return (
    ...
    >
      ...
      {!isMine && (
        <div className="flex-shrink-0 size-7">
      ...
      <div className={cn("flex max-w-[65%] flex-col gap-0.5", isMine ? "items-end" : "items-start")}>
        <div
          className={cn(
        ...
        {message.reactions && message.reactions.length > 0 && (
          ...
          >
            {message.reactions.map((reaction, idx) => (
              <span
                key={idx}
                className="flex items-center gap-0.5 rounded-full bg-secondary px-1.5 py-0.5 text-xs shadow-sm border border-border"
              >
