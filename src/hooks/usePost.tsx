...
export function useSuggestedFeed(userId: number) {
...
export function useLikePost() {
    ...
    const like = async (postId: number, userId: number): Promise<boolean> => {
        ...
        try {
            const success = await postService.create(`${API.POST.BASE}/${postId}/like`, { userId });
            if (!success) {
                dispatch(postActions.updateLikeCount({ postId, delta: -1 }));
            }
            return success;
    ...
    const unlike = async (postId: number, userId: number): Promise<boolean> => {
        ...
        dispatch(postActions.updateLikeCount({ postId, delta: -1 }));
        try {
            const success = await postService.deleteWithBody(`${API.POST.BASE}/${postId}/like`, { userId });
            if (!success) {
                dispatch(postActions.updateLikeCount({ postId, delta: 1 }));
            }
            return success;
